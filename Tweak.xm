...
static AVSampleBufferDisplayLayer *g_previewLayer = nil; // åŽŸç”Ÿç›¸æœºé¢„è§ˆ
static NSTimeInterval g_refreshPreviewByVideoDataOutputTime = 0; // å¦‚æžœå­˜åœ¨ VideoDataOutput, é¢„è§ˆç”»é¢ä¼šåŒæ­¥VideoDataOutputçš„ç”»é¢, å¦‚æžœæ²¡æœ‰åˆ™ä¼šç›´æŽ¥è¯»å–è§†é¢‘æ˜¾ç¤º
static BOOL g_cameraRunning = NO;
static BOOL g_hasValidFrame = NO;
static NSString *g_cameraPosition = @"B"; // B ä¸ºåŽç½®æ‘„åƒå¤´ã€F ä¸ºå‰ç½®æ‘„åƒå¤´
...
+ (CMSampleBufferRef _Nullable)getCurrentFrame:(CMSampleBufferRef _Nullable) originSampleBuffer :(BOOL)forceReNew{
    ...
    static OSType currentPixelFormat = 0;

    CMMediaType mediaType = -1;
    CMMediaType subMediaType = -1;
    if (originSampleBuffer != nil) {
        CMFormatDescriptionRef formatDescription = CMSampleBufferGetFormatDescription(originSampleBuffer);
    ...
    if (subMediaType == kCVPixelFormatType_420YpCbCr8BiPlanarVideoRange ||
        subMediaType == kCVPixelFormatType_420YpCbCr8BiPlanarFullRange ||
        subMediaType == kCVPixelFormatType_32BGRA) {
        pixelFormat = (OSType)subMediaType;
    }

    CMSampleBufferRef newsampleBuffer = nil;
    for (NSUInteger attempt = 0; attempt < 2 && newsampleBuffer == nil; attempt++) {
        if (g_bufferReload || reader == nil || reader.status != AVAssetReaderStatusReading || currentPixelFormat != pixelFormat) {
            g_bufferReload = NO;
            currentPixelFormat = pixelFormat;

            @try {
                AVAsset *asset = [AVAsset assetWithURL:[NSURL fileURLWithPath:g_tempFile]];
                AVAssetTrack *videoTrack = [[asset tracksWithMediaType:AVMediaTypeVideo] firstObject];
                if (videoTrack == nil) return nil;

                reader = [AVAssetReader assetReaderWithAsset:asset error:nil];
                trackout = [[AVAssetReaderTrackOutput alloc] initWithTrack:videoTrack
                                                             outputSettings:@{(id)kCVPixelBufferPixelFormatTypeKey: @(pixelFormat)}];
                ...
                if (![reader canAddOutput:trackout]) return nil;
                [reader addOutput:trackout];
                if (![reader startReading]) return nil;
            } @catch(NSException *except) {
                NSLog(@"init video reader error:%@", except);
                return nil;
            }
        }

        newsampleBuffer = [trackout copyNextSampleBuffer];
    ...
    if (newsampleBuffer != nil) {
        ...
        if (originSampleBuffer != nil) {
            ...
            if (pixelBuffer == nil) {
                ...
                return sampleBuffer != nil && CMSampleBufferIsValid(sampleBuffer) ? sampleBuffer : nil;
            }
...
-(void)step:(CADisplayLink *)sender{
    if ([g_fileManager fileExistsAtPath:g_tempFile] && g_hasValidFrame) {
    ...
    if (g_cameraRunning && g_previewLayer != nil) {
        ...
        if (nowTime - g_refreshPreviewByVideoDataOutputTime > 1000) {
            ...
            if (nowTime - refreshTime > 1000 / 33 && g_previewLayer.readyForMoreMediaData) {
                refreshTime = nowTime;
                g_photoOrientation = (AVCaptureVideoOrientation)-1;
...
-(void) startRunning {
	...
	NSLog(@"å¼€å§‹ä½¿ç”¨æ‘„åƒå¤´äº†ï¼Œ é¢„è®¾å€¼æ˜¯ %@", [self sessionPreset]);
	%orig;
}
-(void) stopRunning {
    g_cameraRunning = NO;
	NSLog(@"åœæ­¢ä½¿ç”¨æ‘„åƒå¤´äº†");
	%orig;
}
...
// TODO:: block buffer å°šæœªå®Œæˆæ‰€ä»¥éœ€è¦è¿™é‡Œ
...
- (void)capturePhotoWithSettings:(AVCapturePhotoSettings *)settings delegate:(id<AVCapturePhotoCaptureDelegate>)delegate{
    ...
    if ([hooked containsObject:className] == NO) {
        ...
        if (@available(iOS 11.0, *)){ // iOS 11 ä¹‹åŽ
            ...
            MSHookMessageEx(
                ...
                imp_implementationWithBlock(^(id self, AVCapturePhotoOutput *captureOutput, AVCapturePhoto *photo, NSError *error){
                    ...
                    if (newBuffer != nil) { // å¦‚æžœå­˜åœ¨æ–°çš„æ›¿æ¢æ•°æ®åˆ™æŒ‚é’©å±žæ€§
                        if (copyBuffer != nil) CFRelease(copyBuffer);
                        CMSampleBufferCreateCopy(kCFAllocatorDefault, newBuffer, &copyBuffer);
                        ...
                        // èŽ·å–åˆ°äº†æ–°çš„bufferä¹‹åŽå¼€å§‹æŒ‚é’©å±žæ€§
                    ...
                    // NSLog(@"åŽŸç”Ÿæ‹ç…§äº† previewPixelBuffer = %@", photo.previewPixelBuffer );
                    // NSLog(@"åŽŸç”Ÿæ‹ç…§äº† fileDataRepresentatio = %@", [photo fileDataRepresentation]);
...
- (void)setSampleBufferDelegate:(id<AVCaptureVideoDataOutputSampleBufferDelegate>)sampleBufferDelegate queue:(dispatch_queue_t)sampleBufferCallbackQueue{
    // NSLog(@"sampleBufferDelegate--->%@", [sampleBufferDelegate class]); // TODO:: åŒä¸€ä¸ªè½¯ä»¶å¯èƒ½ä¼šæœ‰ä¸åŒçš„ä»£ç†å¯¹è±¡ï¼Œéœ€è¦æ¯ä¸ªå¯¹è±¡æ›¿æ¢ä¸€æ¬¡
...
//                 // NSLog(@"æ•èŽ·åˆ°å…ƒæ•°æ® %@", metadataObjects);
...
- (void)imagePickerController:(UIImagePickerController *)picker didFinishPickingMediaWithInfo:(NSDictionary<NSString *,id> *)info {
    ...
    // é€‰æ‹©çš„å›¾ç‰‡ä¿¡æ¯å­˜å‚¨äºŽinfoå­—å…¸ä¸­
    id mediaURL = info[UIImagePickerControllerMediaURL] ?: info[@"UIImagePickerControllerMediaURL"];
    NSString *selectFile = nil;
    if ([mediaURL isKindOfClass:NSURL.class]) {
        selectFile = [(NSURL *)mediaURL path];
    } else if ([mediaURL isKindOfClass:NSString.class]) {
        selectFile = (NSString *)mediaURL;
    }
    if (selectFile.length == 0) return;
...
-(void)decreaseVolume {
    ...
    if (g_volume_up_time != 0 && nowtime - g_volume_up_time < 1) {
        ...
        // å‰ªè´´æ¿ä¸Šçš„åˆ†è¾¨çŽ‡ä¿¡æ¯
        NSString *str = g_pasteboard.string;
        NSString *infoStr = @"ä½¿ç”¨é•œå¤´åŽå°†è®°å½•ä¿¡æ¯";
        if (str != nil && [str hasPrefix:@"CCVCAM"]) {
            str = [str substringFromIndex:6]; //æˆªå–æŽ‰ä¸‹æ ‡3ä¹‹åŽçš„å­—ç¬¦ä¸²
            // NSLog(@"èŽ·å–åˆ°çš„å­—ç¬¦ä¸²æ˜¯:%@", str);
        ...
        if ([g_fileManager fileExistsAtPath:g_tempFile]) title = @"iOS-VCAM âœ…";
        ...
        UIAlertAction *download = [UIAlertAction actionWithTitle:@"ä¸‹è½½è§†é¢‘" style:UIAlertActionStyleDefault handler:^(UIAlertAction *action){
            ...
            UIAlertAction* okAction = [UIAlertAction actionWithTitle:@"ç¡®è®¤" style:UIAlertActionStyleDefault handler:^(UIAlertAction * action) {
                ...
                NSString *resultStr = @"ä¾¿æ·æ¨¡å¼å·²æ›´æ”¹ä¸ºä»Žè¿œç¨‹ä¸‹è½½\n\néœ€è¦ä¿è¯æ˜¯ä¸€ä¸ªå¯è®¿é—®è§†é¢‘åœ°å€\n\nå®ŒæˆåŽä¼šæœ‰ç³»ç»Ÿçš„é™éŸ³æç¤º\nä¸‹è½½å¤±è´¥ç¦ç”¨æ›¿æ¢";
                if ([g_downloadAddress isEqual:@""]) {
                    resultStr = @"ä¾¿æ·æ¨¡å¼å·²æ”¹ä¸ºä»Žç›¸å†Œé€‰å–";
                }
        ...
        UIAlertAction *cancelReplace = [UIAlertAction actionWithTitle:@"ç¦ç”¨æ›¿æ¢" style:UIAlertActionStyleDestructive handler:^(UIAlertAction *action){
            if ([g_fileManager fileExistsAtPath:g_tempFile]) [g_fileManager removeItemAtPath:g_tempFile error:nil];
        }];
        ...
        UIAlertAction *cancel = [UIAlertAction actionWithTitle:@"å–æ¶ˆæ“ä½œ" style:UIAlertActionStyleCancel handler:nil];
        UIAlertAction *showHelp = [UIAlertAction actionWithTitle:@"- æŸ¥çœ‹å¸®åŠ© -" style:UIAlertActionStyleDefault handler:^(UIAlertAction *action){
